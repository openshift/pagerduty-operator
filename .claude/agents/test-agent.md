...
Automated testing and test quality assurance for the PagerDuty Operator.
...
- **controller-runtime fake client**: Kubernetes API simulation
...
for i in {1..5}; do go test ./pkg/pagerduty/ || break; done

# Verbose output
go test -v -run TestMyFunction ./pkg/pagerduty/

# Race detector
go test -race ./pkg/pagerduty/
```
...
1. Check if interface changed in `pkg/pagerduty/service.go`
2. Regenerate mocks: `boilerplate/_lib/container-make generate`
   - Mocks are at `pkg/pagerduty/mock_service.go`
...
- Error path testing
- Edge case coverage
- Mock-based isolation from Kubernetes and PagerDuty API

Don't test:
- Generated code (`zz_generated.*.go`, `mock_service.go`)
