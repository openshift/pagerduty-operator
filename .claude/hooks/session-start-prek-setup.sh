...
# Use git rev-parse --git-path to find the correct hooks path
# This works correctly in both regular repos and git worktrees
HOOKS_DIR=$(git rev-parse --git-path hooks 2>/dev/null || echo "")
PRE_COMMIT_HOOK="${HOOKS_DIR}/pre-commit"

if [[ -z "$HOOKS_DIR" ]] || [[ ! -f "$PRE_COMMIT_HOOK" ]] || ! grep -q "prek" "$PRE_COMMIT_HOOK" 2>/dev/null; then
