...
if [[ -z "$CHANGED_FILES" ]]; then
  ...
  # Pass changed files explicitly to prek using a null-delimited approach
  # to safely handle filenames with spaces
  PREK_OUTPUT=$(echo "$CHANGED_FILES" | while IFS= read -r f; do printf '%s\0' "$f"; done | xargs -0 prek run --config hack/prek.ci.toml --files 2>&1)
fi
PREK_EXIT=$?
